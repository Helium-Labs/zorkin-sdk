...
export async function getNonce(sessionPK, expValidity) {
    const structABICodec = algosdk.ABIType.from('(byte[32],uint64)');
    const unhashedNonce = structABICodec.encode([sessionPK, expValidity]);
...
export function getTealInstanceFromTemplateMap(templateMap, source) {
    // get copy of source
