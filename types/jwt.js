...
export default class OIDCClient {
    ...
    async initialize() {
        ...
        if (extractError() !== null) {
            window?.parent?.postMessage({ eventname: 'jwt', error: extractError() }, window.location.origin);
            window.close();
        }
        const token = extractToken();
        const authCodeAndState = extractAuthCodeAndState();
        if (token !== null) {
            window.opener.postMessage({ eventname: 'jwt', jwt: token }, window.location.origin);
            ...
            window.parent.postMessage({ eventname: 'jwt', jwt: token }, window.location.origin);
            window.close();
        }
        if (authCodeAndState !== null) {
            window.opener.postMessage({ eventname: 'code', codePayload: JSON.stringify(authCodeAndState) }, window.location.origin);
            // also post the message to any parent iframe listener, as this may be within an iframe to initiate silent oauth2 to the parent
            window.parent.postMessage({ eventname: 'code', codePayload: JSON.stringify(authCodeAndState) }, window.location.origin);
            window.close();
        }
    }
    async waitForNextJWT() {
    ...
    async getJWT(loginURL, withoutSilentAuth = false, ignoreCache = false) {
    ...
    async loginWithOIDC({ provider, nonce, ignoreCache = false }) {
